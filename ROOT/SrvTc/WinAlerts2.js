...
function ShowAlerts()
{
	...
	if(Altrafltr.length>=1)
	{
		out+="<input type=\"button\" class=\"INTEXT2\" value=\""+Str_Delet+" Resultado del filtro\" onclick=\"GetUrlB('./setitems.jsp?sql=DELETE FROM alerts WHERE"
		for(var i=0;i<Altrafltr.length;i++)
		{
			if(i==0)
				out+="%20description LIKE %27%25"+Altrafltr[i]+"%25%27"
			else
				out+="%20OR description LIKE %27%25"+Altrafltr[i]+"%25%27"
		}
		out+="',fncnone);return false;\" />\n";
	}
	out+="</td>\n";
	out+="<td align=\"center\">\n";
