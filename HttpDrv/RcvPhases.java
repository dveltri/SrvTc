...
public class RcvPhases
{
	...
	public RcvPhases(String str,String address,String resource) throws Exception
	{
		Connection c = null;
    Statement stmt = null;
		...
		//-----------------------------------------
		if(str.length()%56!=0)
			System.out.println("Leng"+str.length()+"%56="+(str.length()%56));
		//else
		{
			while(str.length()>=56)
			{
				...
				if(Plc>0 && Plc<5)
				{
					...
					try
					{
						sql = "INSERT INTO variables (id,lstchg,value) VALUES (\'/"+address+"/PLC"+Plc+"/Phase"+phase+"/Color\',LOCALTIMESTAMP,\'"+color+"\');"; 
						stmt.executeUpdate(sql);
					}
					catch ( Exception x )
					{
					...
					try
					{
						sql = "INSERT INTO variables (id,lstchg,value) VALUES (\'/"+address+"/PLC"+Plc+"/Phase"+phase+"/Errors\',LOCALTIMESTAMP,\'"+Flags+"\');"; 
						stmt.executeUpdate(sql);
					}
					catch ( Exception x )
					{
